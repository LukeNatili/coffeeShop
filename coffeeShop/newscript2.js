...
function handleFormSubmit(event){
  ...
  // NEW: send to API via AJAX (only if the shopper AJAX block is active on this page)
  if (window.__saveShopperAjax) {
    window.__saveShopperAjax(formData);
  }
}
...
/* === NEW: AJAX (jQuery) for Shopper CRUD ===
   - This keeps your existing validation, but sends the form to your Node/Express API.
   - Endpoints assumed:
       GET    http://localhost:8000/api/shopper
       POST   http://localhost:8000/api/shopper
       PUT    http://localhost:8000/api/shopper/:id
       DELETE http://localhost:8000/api/shopper/:id
   - If your server uses /api/shoppers instead, just change SHOPPER_URL below.
*/
(function () {
  // Only run on pages that actually have the shopper table + form
  if (!form || !tableBody) return;
  if (typeof $ === 'undefined' || !$.ajax) {
    console.error('jQuery is required for AJAX. Make sure jQuery loads before this script.');
    return;
  }

  var SHOPPER_URL = 'http://localhost:8000/api/shopper'; // change to /api/shoppers if needed
  var shopperCache = {}; // id -> shopper (for easy edit)

  function renderShoppers(list) {
    // list should be an array of shoppers from the server
    tableBody.innerHTML = '';
    shopperCache = {};

    if (!Array.isArray(list) || list.length === 0) {
      var emptyRow = document.createElement('tr');
      emptyRow.innerHTML = '<td colspan="10">No shoppers found.</td>';
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      tr.innerHTML =
        ...
        '<td>' + (s.age || '') + '</td>' +
        '<td>' +
          '<button type="button" class="btn btn-sm btn-outline-secondary edit-shopper">Edit</button> ' +
          '<button type="button" class="btn btn-sm btn-outline-danger delete-shopper">Delete</button>' +
        '</td>';
  ...
  function loadShoppers() {
    $.ajax({
      ...
      error: function (jqXHR, textStatus, errorThrown) {
        console.error('Error loading shoppers:', textStatus, errorThrown);
        renderShoppers([]);
      }
    });
  }

  function saveShopper(formData) {
    var id = (editingIndex && editingIndex.value) ? editingIndex.value : '';
    var url = id ? (SHOPPER_URL + '/' + id) : SHOPPER_URL;
    ...
    $.ajax({
      ...
      dataType: 'json',
      success: function () {
        alert(id ? 'Shopper updated!' : 'Shopper saved!');
        if (editingIndex) editingIndex.value = '';
        form.reset();
        loadShoppers();
      },
      error: function (jqXHR, textStatus, errorThrown) {
        console.error('Error saving shopper:', textStatus, errorThrown);
        alert('Could not save shopper. Check your API is running on port 8000.');
      }
    });
  }
  ...
  function deleteShopper(id) {
    ...
    $.ajax({
      ...
      error: function (jqXHR, textStatus, errorThrown) {
        console.error('Error deleting shopper:', textStatus, errorThrown);
        alert('Could not delete shopper.');
      }
    });
  }

  // Hook: when your existing validation passes, this global flag lets us submit via AJAX
  window.__saveShopperAjax = saveShopper;
  ...
  tableBody.addEventListener('click', function (e) {
    ...
    if (btn.classList.contains('delete-shopper')) {
      if (confirm('Delete this shopper?')) deleteShopper(id);
      return;
    }

    if (btn.classList.contains('edit-shopper')) {
      var s = shopperCache[id];
      if (!s) return;

      // Fill the form inputs
      if (inputs.firstName) inputs.firstName.value = s.firstName || '';
      if (inputs.lastName) inputs.lastName.value = s.lastName || '';
      if (inputs.email) inputs.email.value = s.email || '';
      if (inputs.phone) inputs.phone.value = s.phone || '';
      if (inputs.address) inputs.address.value = s.address || '';
      if (inputs.city) inputs.city.value = s.city || '';
      if (inputs.state) inputs.state.value = s.state || '';
      if (inputs.zip) inputs.zip.value = s.zip || '';
      if (inputs.age) inputs.age.value = s.age || '';

      if (editingIndex) editingIndex.value = id; // stores the id for PUT
      window.scrollTo(0, 0);
    }
  });

  // Optional: local search filter (no extra server code needed)
  if (search) {
